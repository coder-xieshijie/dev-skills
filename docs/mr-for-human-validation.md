...
最新修订见第 7 节。第 1–6 节保留历史检查记录，其中的示例章节、P/I 编号、输出格式及当时的客户端状态不代表当前版本。
...
日期：2026-09-15。依据为用户本轮要求：金字塔贯穿始终，只决策重要事项；关注目录具体修改以识别修复之外的内容；解释边界和失败降级。

### 改动范围

| 目录 / 文件 | 具体修改及其必要性 |
|---|---|
| skills/mr-for-human/SKILL.md | 声明三原则，收敛为四步；逐文件范围核对与失败收尾保留为完成条件，六维转为内部检查 |
| skills/mr-for-human/references/output-template.md | 简化为结论、目录具体改动、流程与边界降级、阅读及验证四部分 |
| skills/mr-for-human/references/design-lenses.md | 移除强制四句式，补充降级触发、隔离、再次失败与恢复责任的证据要求 |
| skills/mr-for-human/references/worked-example.md | 修复入口成本、依赖隐藏与系统能力的过度推断；压缩原示例，补一个正常降级示例 |
| docs/mr-for-human-design.md | 标记旧方案为历史，指向当前规则，保留原始依据与取舍记录 |
| docs/mr-for-human-validation.md | 记录本轮实际验证及限制 |
| README.md | 同步使用说明及当前工作树修订状态 |

上述文件均为本 Skill 的入口、配套资料或导航，无其他 Skill 改动；客户端配置与软链接未修改。

### 实际验证

- `quick_validate.py skills/mr-for-human` 通过；Git 空白检查通过。
- 7 个相关 Markdown 文件均可按 UTF-8 读取，相对文件链接有效、代码围栏闭合；6 个 Python 代码块可编译。
- 从当前教学文档提取 Python 代码，在内存中配合 Store/Broker/Reader 测试替身执行，无外部服务调用。原示例的 before/after 源码与修订前逐字一致。

| 示例 | 已执行场景 | 观察 |
|---|---|---|
| 任务查重 | 拒绝无权限调用、正常串行重试、不同租户同键、发布失败后新版本重试、相同失败的旧版本对照 | 5 项通过；新版本重试返回 ACCEPTED 但消息仍为 0，旧版本重试产生第二条记录并发布 |
| 目录降级 | 正常读取、超时回退及提示、权限异常、备用读取失败、租户参数传递、下一次调用恢复正常读取 | 6 项通过；仅超时进入回退，其他错误传播；回退结果带“备用数据”提示，后续仍先尝试远端 |

### 场景走查与限制

对本轮真实修改按文件核对范围，结果见上表。另按指令静态走查了两类边界：

- 修复所在文件同时修改共享默认值：应分别解释这两项行为，默认值变化的关系不能被整个文件的“目标内修改”覆盖。
- 只有文件列表和截断 diff：不可见文件应进入“尚未读取/不可见”，输出仅声明已读范围；不把它们归为次要，也不声称扫描了全部语义变化。

上述走查由编写者进行，未独立运行 Agent 生成指南。执行测试只验证教学模型，未验证真实存储约束、网络超时取消、数据隔离实现或 UI 集成；租户测试证明的是参数传递及测试替身行为。尚无真实读者效果数据。当前修订留在本工作树，未同步到已安装 Skill 的维护源；本轮为纯文档修改，未查询 CI。
