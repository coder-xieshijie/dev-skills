...
2. **目录改动可核对。**按目录展示具体文件改了什么、为什么与本次目标有关。每个变更文件都须有去向，目录摘要和重点选择不能隐藏额外行为。尤其检查混入的清理、重构、默认值调整与共享行为变化；关系无法证明的单独标注。
...
完成条件：分析快照和目标明确，范围内所有已知变更文件均有记录。材料不全时区分“已读但未展开”和“尚未读取/不可见”；只对已读取范围给结论，保留缺口并继续可完成的分析。
...
完成条件：关键行为与失败后果有源码或契约依据；无法补足的链路明确停在证据边界，可继续形成有限指南。
...
- 按已知变更清单逐文件对账，确认目录说明覆盖具体变化；额外改动和待确认项没有被“必要配套”掩盖。
