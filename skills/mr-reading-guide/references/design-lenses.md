...
**这张表不是封闭清单。**它收录的是高频且有一手依据的维度。遇到表外的约束，按 SKILL.md 第 3.3 节的四句式同样处理——做了什么、依赖什么成立、证据在哪、依赖不成立会怎样；不要因为表里没有就跳过。
...
六项默认必查，各对应一条绕不过去的事实。第四列是“第 2 句的依赖不成立时会发生什么”，用来写四句式的末句，不是预设本次改动有缺陷。
...
| 系统事实需要被观察和恢复 | 执行、持久化、通知、UI 显示是否混为一个状态？如何定位丢失的一步？ | 状态字段、关联 ID、事件、日志、恢复与回滚流程 | 故障静默发生，外部看起来完全正常，因此没人去修；代码回退不自动撤销已写数据或已发生的外部副作用 |
...
本次改动碰到才展开。它们多数是上面六项的高频具体形态，单列是因为各有独立的证据要求；不适用时一句依据带过，不需要逐条论证。
